...
import cv2
import numpy as np
from nn import NN_ResNet50_bnf
import models
...
if __name__ == '__main__':
    if args.arch == 'alex':
        ...
        model = L.Classifier(NN_ResNet50_bnf(256, 2, 4))
    ...
    gcam = grad_cam.generate(x, args.label, model.predictor.resnet50.conv5_3)
    gcam = np.uint8(gcam * 255 / gcam.max())
    gcam = cv2.resize(gcam, (model.size, model.size))
    gbp = guided_backprop.generate(x, args.label, model.predictor.resnet50.conv5_3)
    ...
    cv2.imwrite(args.output_name, gcam)
